...
extension MLNStyleLayerExt on MLNStyleLayer {
  ...
  void setProperties(Map<String, Object> properties) {
    for (final property in properties.entries) {
      try {
        ...
        debugPrint('Error setting property ${property.key}: $e');
  ...
  void setProperty(String key, Object value) {
    try {
      // Special handling for line-dasharray
      if (key == 'line-dasharray' && value is List<num>) {
        final lineLayer = this as MLNLineStyleLayer;
        try {
          final dashValues = value.map((n) => n.toDouble()).toList();
          final arrayLiteral = '{${dashValues.join(', ')}}';
          final expression = NSExpression.expressionWithFormat_(arrayLiteral.toNSString());
          lineLayer.lineDashPattern = expression;
          debugPrint('Successfully set dash array: $dashValues');
          return;
        } catch (e) {
          debugPrint('Failed to set line-dasharray: $e');
          return;
        }
      }
      ...
      try {
        ...
        debugPrint('Error parsing expression for $key: $error');
        return;
      }

      if (expression == null) {
        debugPrint('Expression is null for $key');
        return;
      }

      // Set the property based on the key
      _setPropertySafely(key, expression);

    } catch (e) {
      debugPrint('Error in setProperty for $key: $e');
    }
  }

  void _setPropertySafely(String key, NSExpression expression) {
    try {
      switch (key) {
        ...
        case 'fill-extrusion-vertical-gradient':
          (this as MLNFillExtrusionStyleLayer).fillExtrusionHasVerticalGradient = expression;
        case 'fill-extrusion-translate':
          (this as MLNFillExtrusionStyleLayer).fillExtrusionTranslation = expression;
        case 'fill-extrusion-translate-anchor':
          (this as MLNFillExtrusionStyleLayer).fillExtrusionTranslationAnchor = expression;
        case 'line-dasharray':
      ...
      debugPrint('Error setting property $key: $e');
    }
  }
}
