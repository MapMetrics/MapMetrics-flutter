...
protocol MapLibreHostApi {
  ...
  func addVectorTileSource(sourceId: String, tileUrls: [String], minZoom: Int64, maxZoom: Int64, completion: @escaping (Result<Int64, Error>) -> Void)
  ...
  func setSymbolLayerColors(layerId: String, iconColor: String?, iconHaloColor: String?, textColor: String?, textHaloColor: String?, completion: @escaping (Result<Void, Error>) -> Void)
}
...
class MapLibreHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: MapLibreHostApi?, messageChannelSuffix: String = "") {
    ...
    /// This bypasses Dart's NSNumber creation limitation by handling everything in Swift.
    /// Returns the pointer address of the created MLNVectorTileSource as an int.
    let addVectorTileSourceChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.addVectorTileSource\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      addVectorTileSourceChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let sourceIdArg = args[0] as! String
        let tileUrlsArg = args[1] as! [String]
        let minZoomArg = args[2] as! Int64
        let maxZoomArg = args[3] as! Int64
        api.addVectorTileSource(sourceId: sourceIdArg, tileUrls: tileUrlsArg, minZoom: minZoomArg, maxZoom: maxZoomArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      addVectorTileSourceChannel.setMessageHandler(nil)
    }
    /// Set color properties on a symbol layer safely in Swift.
    /// This bypasses Dart's color parsing issues by handling MGLColor creation in Swift.
    /// Colors should be hex strings with alpha channel (#RRGGBBAA format).
    let setSymbolLayerColorsChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.setSymbolLayerColors\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      setSymbolLayerColorsChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let layerIdArg = args[0] as! String
        let iconColorArg: String? = nilOrValue(args[1])
        let iconHaloColorArg: String? = nilOrValue(args[2])
        let textColorArg: String? = nilOrValue(args[3])
        let textHaloColorArg: String? = nilOrValue(args[4])
        api.setSymbolLayerColors(layerId: layerIdArg, iconColor: iconColorArg, iconHaloColor: iconHaloColorArg, textColor: textColorArg, textHaloColor: textHaloColorArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
      ...
      setSymbolLayerColorsChannel.setMessageHandler(nil)
    }
  }
}
