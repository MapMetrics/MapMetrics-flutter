...
interface MapLibreHostApi {
   ...
   */
  fun addVectorTileSource(sourceId: String, tileUrls: List<String>, minZoom: Long, maxZoom: Long, callback: (Result<Long>) -> Unit)
  /**
   ...
   */
  fun setSymbolLayerColors(layerId: String, iconColor: String?, iconHaloColor: String?, textColor: String?, textHaloColor: String?, callback: (Result<Unit>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MapLibreHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.addVectorTileSource$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val sourceIdArg = args[0] as String
            val tileUrlsArg = args[1] as List<String>
            val minZoomArg = args[2] as Long
            val maxZoomArg = args[3] as Long
            api.addVectorTileSource(sourceIdArg, tileUrlsArg, minZoomArg, maxZoomArg) { result: Result<Long> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.setSymbolLayerColors$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val layerIdArg = args[0] as String
            val iconColorArg = args[1] as String?
            val iconHaloColorArg = args[2] as String?
            val textColorArg = args[3] as String?
            val textHaloColorArg = args[4] as String?
            api.setSymbolLayerColors(layerIdArg, iconColorArg, iconHaloColorArg, textColorArg, textHaloColorArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
