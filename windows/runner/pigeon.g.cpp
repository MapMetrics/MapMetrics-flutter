...
void MapLibreHostApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.addVectorTileSource" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          ...
          const auto& source_id_arg = std::get<std::string>(encodable_source_id_arg);
          const auto& encodable_tile_urls_arg = args.at(1);
          if (encodable_tile_urls_arg.IsNull()) {
            reply(WrapError("tile_urls_arg unexpectedly null."));
            return;
          }
          const auto& tile_urls_arg = std::get<EncodableList>(encodable_tile_urls_arg);
          ...
          const int64_t min_zoom_arg = encodable_min_zoom_arg.LongValue();
          ...
          const int64_t max_zoom_arg = encodable_max_zoom_arg.LongValue();
          api->AddVectorTileSource(source_id_arg, tile_urls_arg, min_zoom_arg, max_zoom_arg, [reply](ErrorOr<int64_t>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.mapmetrics.MapLibreHostApi.setSymbolLayerColors" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          const auto& encodable_layer_id_arg = args.at(0);
          if (encodable_layer_id_arg.IsNull()) {
            reply(WrapError("layer_id_arg unexpectedly null."));
            return;
          }
          const auto& layer_id_arg = std::get<std::string>(encodable_layer_id_arg);
          const auto& encodable_icon_color_arg = args.at(1);
          const auto* icon_color_arg = std::get_if<std::string>(&encodable_icon_color_arg);
          const auto& encodable_icon_halo_color_arg = args.at(2);
          const auto* icon_halo_color_arg = std::get_if<std::string>(&encodable_icon_halo_color_arg);
          const auto& encodable_text_color_arg = args.at(3);
          const auto* text_color_arg = std::get_if<std::string>(&encodable_text_color_arg);
          const auto& encodable_text_halo_color_arg = args.at(4);
          const auto* text_halo_color_arg = std::get_if<std::string>(&encodable_text_halo_color_arg);
          api->SetSymbolLayerColors(layer_id_arg, icon_color_arg, icon_halo_color_arg, text_color_arg, text_halo_color_arg, [reply](std::optional<FlutterError>&& output) {
            if (output.has_value()) {
              reply(WrapError(output.value()));
              return;
            }
